package com.thinkit.cloud.flows.api;
...
import com.thinkit.cloud.flows.AssignmentHandler;
...
public class FlowTaskServiceApiImpl implements FlowTaskServiceApi {
  ...
  public FlowTask complete(String taskId, String operator, Map<String, Object> args) throws Exception {
    FlowTask task = (FlowTask) flowEngineFacets.getEngine().flowTaskService().selectByPrimaryKey(Long.parseLong(taskId));
   ...
   // flowTaskHist.setStatus(FlowConstains.STATE_FINISH);
    ...
    if (flowTaskHist.getActorIds() != null) {
      for (String actorId : flowTaskHist.getActorIds()) {
        ...
        flowEngineFacets.getEngine().flowTaskActorHistService().insert(hist);// 历史参与者
      }
    }

    flowEngineFacets.getEngine().flowTaskHistService().insert(flowTaskHist);
    flowEngineFacets.getEngine().flowTaskService().deleteByPrimaryKey(task.getId());
 ...
 //   historyTask.setTaskType(TaskType.Record.ordinal());
    java.lang.Long parentTaskId = execution.getTask() == null ? null : execution.getTask().getId();
    historyTask.setParentTaskId(parentTaskId);
    historyTask.setVariable(JsonUtil.toJson(execution.getArgs()));
    flowEngineFacets.getEngine().flowTaskHistService().insert(historyTask);
    return historyTask;
  ...
  public FlowTask take(String taskId, String operator) throws Exception {
    ...
    flowEngineFacets.getEngine().flowTaskService().updateByPrimaryKey(task);
    return task;
  }
  ...
  public FlowTask resume(String taskId, String operator) throws Exception {
    FlowTaskHist flowTaskHist = (FlowTaskHist) flowEngineFacets.getEngine().flowTaskHistService().selectByPrimaryKey(Long.parseLong(taskId));
    ...
    if (isAllowed) {
      ...
      flowEngineFacets.getEngine().flowTaskService().insert(task);
   ...
   */
  public void addTaskActor(String taskId, Long performType, String... actors) throws Exception {
    FlowTask flowTask = (FlowTask) flowEngineFacets.getEngine().flowTaskService().selectByPrimaryKey(Long.parseLong(taskId));
    Assert.notNull(flowTask, "指定的任务[id=" + taskId + "]不存在");
    if (!flowTask.isMajor())  {
      return;
    }
    if (performType == null) {
      performType = flowTask.getPerformType();
    }
    if (performType == null)  {
      performType = 0L;
    ...
    if (performType == 0)  {
        ...
        flowEngineFacets.getEngine().flowTaskService().updateByPrimaryKey(flowTask);
    }
    if (performType == 1)  {
        try {
            for (String actor : actors) {
              ...
              Long newTaskid = flowEngineFacets.getEngine().flowTaskService().insert(newTask);
  ...
  public FlowTask withdrawTask(String taskId, String operator) throws Exception {
    FlowTaskHist hist = (FlowTaskHist) flowEngineFacets.getEngine().flowTaskHistService().selectByPrimaryKey(Long.parseLong(taskId));
    ...
    task.setCreateTime(new Date());
    Long id = flowEngineFacets.getEngine().flowTaskService().insert(task);
  ...
  public FlowTask rejectTask(ProcessModel model, FlowTask currentTask) throws Exception {
    ...
    FlowTaskHist history = (FlowTaskHist) flowEngineFacets.getEngine().flowTaskHistService().selectByPrimaryKey(Long.parseLong(parentTaskId));
  ...
  private void assignTask(String taskId, String... actorIds) throws Exception {
    ...
    for (String actorId : actorIds) {
      ...
      taskActor.setTaskId(Long.parseLong(taskId));
      taskActor.setActorId(actorId);
      flowEngineFacets.getEngine().flowTaskActorService().insert(taskActor);
    }
  }
   ...
   */
  public List<FlowTask> createNewTask(String taskId, String taskType, String... actors) throws Exception {
    FlowTask flowTask = (FlowTask) flowEngineFacets.getEngine().flowTaskService().selectByPrimaryKey(Long.parseLong(taskId));
    Assert.notNull(flowTask, "指定的任务[id=" + taskId + "]不存在");
    List<FlowTask> tasks = new ArrayList<FlowTask>();
    try {
      FlowTask newTask = (FlowTask) flowTask.clone();
      newTask.setTaskType(Long.parseLong(taskType));
      newTask.setCreateTime(new Date());
      newTask.setParentTaskId(Long.parseLong(taskId));
  ...
  public TaskModel getTaskModel(String taskId) throws Exception {
    ...
    FlowOrder order = (FlowOrder) flowEngineFacets.getEngine().flowOrderService()
        .selectByPrimaryKey(flowTask.getFlowOrderId());
    Assert.notNull(order);
    FlowProcess process = (FlowProcess) flowEngineFacets.getEngine().flowProcessService()
        .selectByPrimaryKey(order.getFlowProcessId());
 ...
 //   task.setPerformType(PerformType.ANY.ordinal());
  ...
  public boolean isAllowed(FlowTask task, String operator) {
    ...
    List<FlowTaskActor> actors = flowEngineFacets.getEngine().flowTaskActorService()
        .getTaskActorsByTaskId(String.valueOf(task.getId()));
    if (actors == null || actors.isEmpty()) {
      return true;
    }
    return !StringUtil.isBlank(operator)
        && flowEngineFacets.getEngine().flowTaskAccessStrategyService().isAllowed(operator, actors);
  }

  /**
   * 获得下一个流程
   * 
   * @param parentTaskId 流程任务Id
   * @return 获得下一个流程
   * @throws Exception 异常
   */
  public List<FlowTask> getNextActiveTasks(Long parentTaskId) throws Exception {
    return flowEngineFacets.getEngine().flowTaskService().selectByExample(new Query().putFilter("parentTaskId",parentTaskId));
  }

  /**
   * 从历史表获得下一个流程 @param parentTaskId @return @throws
   */
  public List<FlowTask> getNextActiveTasks(Long flowOrderId, String taskName, Long parentTaskId)
      throws Exception {
    Map<String, Object> map = new HashMap<String, Object>();
    map.put("flowOrderId", flowOrderId);
    map.put("taskName", taskName);
    map.put("parentTaskId", parentTaskId);
    //Fixme getNextActiveTasks
